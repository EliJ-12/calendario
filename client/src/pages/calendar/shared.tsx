...
import { Calendar, ChevronLeft, ChevronRight, MessageCircle, X, Edit2, Trash2, User, Clock } from "lucide-react";
...
const EVENT_CATEGORIES = [
  ...
  { value: 'Entrega', color: '#FF6B6B', bgColor: '#FF6B6B10' },
  { value: 'Presentación', color: '#4ECDC4', bgColor: '#4ECDC410' },
  { value: 'Evento trabajo', color: '#45B7D1', bgColor: '#45B7D110' },
  { value: 'Evento universidad', color: '#96CEB4', bgColor: '#96CEB410' }
] as const;
...
export default function SharedCalendar() {
  ...
  // Delete shared event mutation (only for creator)
  const deleteEventMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`/api/shared-events/${id}`, {
        method: 'DELETE'
      });
                        ...
                        <div className="flex-1">
                          ...
                          {event.comments.length > 0 && (
                            <div className="flex items-center gap-1 mt-2">
                              ...
                              <span className="text-xs text-gray-500">{event.comments.length} comentarios</span>
                            </div>
                          )}
                        </div>
                        <div className="flex gap-2">
          ...
          <DialogContent className="max-w-2xl">
            ...
            {selectedEvent && (
              <div className="space-y-4">
                <div className="p-3 border rounded-lg bg-gray-50">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge variant="secondary" style={{ 
                      backgroundColor: getCategoryColor(selectedEvent.category).bgColor, 
                      color: getCategoryColor(selectedEvent.category).color 
                    }}>
                ...
                {/* Comments Section */}
                <div className="space-y-3">
                  <h4 className="font-medium">Comentarios ({selectedEvent.comments.length})</h4>
                    ...
                    {selectedEvent.comments.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {selectedEvent.comments.map(comment => (
                          <div key={comment.id} className="flex gap-3 p-2 border rounded">
                ...
                {/* Add Comment */}
                <div className="space-y-3">
                  <h4 className="font-medium">Añadir Comentario</h4>
                  <div className="flex gap-2">
                    <Textarea
